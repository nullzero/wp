...
-requiretext:XYZ  Only do pages which contain the text XYZ. If the -regex
                  argument is given, XYZ will be regarded as a regular
                  expression.
...
class XmlDumpReplacePageGenerator:
    ...
    def isTextExcepted(self, text):
        ...
        if "require-text" in self.exceptions:
            for req in self.exceptions['require-text']:
                if not req.search(text): # if not all requirements are met:
                    return True
        return False
...
class ReplaceRobot:
    ...
    def __init__(self, generator, replacements, exceptions={},
        ...
        This dictionary can have these keys:
            ...
            require-text
                Opposite of text-contains. Only pages with text that are matched by
                ALL of these regular expressions will be processed.
                This is not an exception, and is here for technical reasons.
                Listing the same regex in text-contains and require-text will thus
                prevent the bot of doing anything.
            include
    ...
    def isTextExcepted(self, original_text):
        ...
        if "require-text" in self.exceptions:
            for req in self.exceptions['require-text']:
                if not req.search(original_text): # if not all requirements are met:
                    return True
        return False
...
def main(*args):
    ...
    for arg in pywikibot.handleArgs(*args):
        ...
        elif arg.startswith('-requiretext:'):
            exceptions['require-text'].append(arg[13:])
    ...
    for exceptionCategory in [
                        'require-text', 'title', 'require-title', 'text-contains', 'inside']:
