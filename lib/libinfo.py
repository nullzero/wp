...
def extract(key, text):
    """
    Extract key from text. Note that both key and text must be unicode strings.
    """
    if key:
        try: dat = preload.enUnicode(re.search(u"(?m)^\* " + re.escape(key) + u": (.*?)$",
            text).group(1))
        except: dat = None
    else:
...
def changeValue(key, value, text):
    ...
    text, changes = re.subn(u"(?m)^\* %s: (.*?)$" % re.escape(key),
                        ...
                        text)
    # don't check that newtext is equal to oldtext because if newvalue is equal oldvalue,
    # it will duplicate that key.
    if changes == 0:
        if text: text = text + u"\n* " + key + u": " + value
        else: text = u"* " + key + u": " + value
